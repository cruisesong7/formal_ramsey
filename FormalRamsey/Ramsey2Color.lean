...
def Ramsey₂Prop (N s t : ℕ) : Prop := RamseyProp N (s ::ᵥ t ::ᵥ List.Vector.nil)

instance : DecidablePred (Membership.mem { N | Ramsey₂Prop N s t }) := by
  simp [Ramsey₂Prop]
  infer_instance
...
def Ramsey₂GraphProp (N s t : ℕ) : Ramsey₂Prop N s t ↔ RamseyGraphProp N s t := by
  ...
  · intros RamseyGraphN f
    ...
    · use S, 0
      simp [SimpleGraph.isNClique_iff, SimpleGraph.IsClique, Set.Pairwise, GAdj] at Sclique ⊢
...
theorem Ramsey₂PropSymm : ∀ {N s t}, Ramsey₂Prop N s t ↔ Ramsey₂Prop N t s := by
  suffices helper : ∀ N s t, Ramsey₂Prop N s t → Ramsey₂Prop N t s by
    intros N s t
    use helper N s t, helper N t s
  simp [Ramsey₂Prop, RamseyProp]
  intros N s t R f
  let f' : Sym2 (Fin N) → Fin 2 := λ e ↦ if f e = 0 then 1 else 0
  rcases (R f') with ⟨S, ⟨SClique, SCard⟩ | ⟨SClique, SCard⟩⟩
  use S; right; swap; use S; left
  all_goals (
    constructor
    simp [SimpleGraph.isClique_iff, Set.Pairwise, graphAtColor, f'] at SClique ⊢
    intros _ xinS _ yinS xneqy
    simp_all
  )
  · simp [List.Vector.get] at SCard
    assumption
  · rw [← not0_eq1]
    apply SClique <;> assumption
  · simp [List.Vector.get, SCard]
...
theorem friendshipUpperbound : Ramsey₂Prop 6 3 3 := by
  ...
  rcases chyp with ⟨x, _, y, _, z, _, ⟨xneqy, xneqz, yneqz⟩⟩
  ...
  · simp at h
    ...
    · simp [not0_eq1] at fxyneqc
      simp [fxyneqc] at d0
      use {↑x, ↑y, ↑z}
      simp[List.Vector.get, d0]
    · simp [← not0_eq1] at fxyneqc
      simp [fxyneqc] at d0
      use {↑x, ↑y, ↑z}
      simp[List.Vector.get, d0]
  simp [Finset.card_pos] at h
  have e := Finset.Nonempty.exists_mem h
  ...
  have hgsum : Finset.univ.sum h = Finset.univ.sum g := by
    ...
    have seteq : (monochromaticVicinity ⊤ 0 f 0) ∪ (monochromaticVicinity ⊤ 0 f 1) = ((⊤:SimpleGraph (Fin (M + N))).neighborFinset 0) := by
      ...
      · simp [monochromaticVicinity]
        intros a ainset
        simp [ainset]
        have funiv := Finset.mem_univ (f s(0, a))
        simp [univ_fin2] at funiv
        assumption
    rw [← Finset.card_union_of_disjoint filterdisj, seteq, SimpleGraph.neighborFinset_eq_filter]
    simp [← SimpleGraph.completeGraph_eq_top, Finset.filter_ne, Int.ofNat_sub MNpos, Rat.mkRat_one]
  have mp := Finset.exists_le_of_sum_le (Exists.intro (0 : Fin 2) (Finset.mem_univ (0 : Fin 2))) (le_of_eq hgsum)
...
theorem Ramsey₂Finite : ∀ s t : ℕ, { N : ℕ | Ramsey₂Prop N s t }.Nonempty := by
  suffices Ramsey₂FiniteAdditive : ∀ m : ℕ, ∀ s t, m = s + t → { N : ℕ | Ramsey₂Prop N s t }.Nonempty by
    intros s t
    ...
    | succ m' ih =>
      ...
      | succ s =>
        ...
        | zero =>
          ...
          intro f
          use {}
          simp [SimpleGraph.isNClique_iff, SimpleGraph.isClique_iff, graphAtColor, List.Vector.get]
        | succ t =>
          ...
          | inl ST0 =>
            ...
            fin_cases i <;> simp [List.Vector.get] at i0
            · use 1
              simp [i0]
              apply Ramsey₂Prop1
          | inr STPos =>
...
def Ramsey₂ (s t : ℕ) : ℕ := Nat.find (Ramsey₂Finite s t)

theorem Ramsey₂2 : ∀ k : ℕ, Ramsey₂ 2 k.succ = k.succ := by
  intros k
  unfold Ramsey₂
  rw [Nat.find_upward_closed_eq_succ_iff (Ramsey₂Finite 2 k.succ)]
  · simp
    apply And.intro
    · simp [Ramsey₂Prop, RamseyProp]
      intros f
      cases Finset.eq_empty_or_nonempty (Finset.filter (λ (x : Sym2 (Fin k.succ)) ↦ ¬ x.IsDiag ∧ f x = 0) Finset.univ) with
      | inl fempty =>
        simp [Finset.filter_eq_empty_iff] at fempty
        use Finset.univ
        right
        constructor
        · simp [graphAtColor]
          intros x _ y _ xneqy
          simp [xneqy, ← not0_eq1]
          intro fsxy
          have nDiag : ¬(s(x, y)).IsDiag  := by simp_all
          have := fempty nDiag
          contradiction
        · simp [List.Vector.get]
      | inr fnonempty =>
        obtain ⟨⟨x, y⟩, wprop⟩ := fnonempty
        simp at wprop
        use {x, y}
        left
        constructor
...
theorem Ramsey₂1 : ∀ k : ℕ, Ramsey₂ 1 k.succ = 1 := by
  intro k
  simp [Ramsey₂]
  rw [Nat.find_upward_closed_eq_succ_iff (Ramsey₂Finite 1 k.succ)]
  · simp
    apply And.intro
    apply Ramsey₂Prop1 0 k.succ
    simp [Ramsey₂Prop, RamseyProp]
    use (λ _ ↦ 0)
    intros S SNClique
    have Sempty : S = ∅ := by simp [Finset.eq_empty_iff_forall_notMem]
    simp [SimpleGraph.isNClique_iff, SimpleGraph.isClique_iff, Set.Pairwise, graphAtColor, Sempty, List.Vector.get] at SNClique
  · intros M₁ M₂ M₁leM₂
    simp
    intro M₁Ramsey
    apply RamseyMonotone M₁Ramsey M₁leM₂
...
theorem Ramsey₂PropStrictIneq : ∀ {M N s t : ℕ}, Odd M → Odd N → Ramsey₂Prop M.succ s t.succ → Ramsey₂Prop N.succ s.succ t → Ramsey₂Prop (M + N).succ s.succ t.succ := by
  ...
  have hgsum : Finset.univ.sum h = Finset.univ.sum g := by
    simp [Finset.univ_fin2, h, g, -SimpleGraph.edgeFinset_top]
    ...
    have filterdisj : Disjoint (Finset.filter (λ e ↦ f e = 0) (⊤ : SimpleGraph (Fin (M + N).succ)).edgeFinset) (Finset.filter (λ e ↦ f e = 1) (⊤ : SimpleGraph (Fin (M + N).succ)).edgeFinset) := by
      ...
      intros a _ fa0 b _ fb1 h
      simp [h,fb1] at fa0
    simp [-SimpleGraph.edgeFinset_top] at filterdisj
    rw [← Finset.card_union_of_disjoint filterdisj]
    have seteq : Finset.filter (λ e ↦ f e = 0) (⊤ : SimpleGraph (Fin (M + N).succ)).edgeFinset ∪ Finset.filter (λ e ↦ f e = 1) (⊤ : SimpleGraph (Fin (M + N).succ)).edgeFinset = (⊤ : SimpleGraph (Fin (M + N).succ)).edgeFinset := by
      simp [Finset.Subset.antisymm_iff, Finset.subset_iff]
      apply And.intro
      intros _ xprop ; cases xprop <;> simp_all
      intros x xprop
      simp [xprop]
      have fx : f x ∈ Finset.univ := by simp
      simp [Finset.univ_fin2] at fx
      exact fx
    rw [seteq, SimpleGraph.card_edgeFinset_top_eq_card_choose_two]
    simp [Nat.choose_two_right]
    symm
    apply Nat.mul_div_cancel'
    rw [Nat.dvd_iff_mod_eq_zero, ← Nat.even_iff, Nat.mul_comm]
    exact Nat.even_mul_succ_self (M + N)
  have mp := Finset.exists_le_of_sum_le (Exists.intro (0 : Fin 2) (Finset.mem_univ (0 : Fin 2))) (Nat.le_of_eq hgsum)
  ...
  have xoreven : ∀ {O : ℕ}, Odd O → Xor' (Even ((M + N).succ * O)) (Even (2 * (Finset.filter (λ (e : Sym2 (Fin (M + N).succ)) ↦ f e = a) (⊤ : SimpleGraph (Fin (M + N).succ)).edgeFinset).card)) := by
    have cardodd := Even.add_one (Odd.add_odd oddM oddN)
    intros O OddO
    right
    simp
    apply Odd.mul <;> assumption
  have cardeq : ∀ (v : Fin (M + N).succ) (i : Fin 2), (Finset.filter (λ (x : (⊤ : SimpleGraph (Fin (M + N).succ)).Dart)↦ x.toProd.snd = v)
    ...
    intro v i
    apply Finset.card_bij (λ (a : (⊤ : SimpleGraph (Fin (M + N).succ)).Dart) _ ↦ a.fst)
    · intro a
      simp [monochromaticVicinity]
      intros fi asndv
      have aprop := a.adj
      ...
      exact fi
    · simp
      intros _ fa asndv _ fb bsndv abfst
  ...
  fin_cases a <;> simp [-SimpleGraph.edgeFinset_top] at gha
  · have ghalt := xor_even_le_implies_lt (xoreven oddM) gha
    simp [-SimpleGraph.edgeFinset_top] at ghalt
    ...
    · use S
      ...
      exact Sclique
    · use S
      simp [notc Sclique.left (Fin.succ_ne_zero 0)] at Sclique
      right
      exact Sclique
  · have ghalt := xor_even_le_implies_lt (xoreven oddN) gha
    ...
    · use T
      ...
      exact Tclique
    · use T
      simp [notc Tclique.left (Fin.succ_ne_zero 0).symm] at Tclique
      left
      exact Tclique
...
theorem Ramsey₂ToRamsey₂Prop : ∀ {N s t : ℕ}, Ramsey₂ s t = N → Ramsey₂Prop N s t := by
  ...
  simp [← Ramsey]
  apply Nat.find_spec (Ramsey₂Finite s t)

theorem Ramsey₂Ineq : ∀ s t : ℕ, 0 < Ramsey₂ s t.succ + Ramsey₂ s.succ t → Ramsey₂ s.succ t.succ ≤ Ramsey₂ s t.succ + Ramsey₂ s.succ t := by
  intros s t RPos
  have RamseyM := Nat.find_spec (Ramsey₂Finite s t.succ)
  have RamseyN := Nat.find_spec (Ramsey₂Finite s.succ t)
  apply Nat.find_le
...
theorem Ramsey₂Symm : ∀  {s t: ℕ}, Ramsey₂ s t = Ramsey₂ t s := by
  ...
  · have RamseyM := Nat.find_spec (Ramsey₂Finite t s)
    apply Nat.find_le
    simp [Ramsey₂] at RamseyM ⊢
    rw [Ramsey₂PropSymm] at RamseyM
    assumption
  · have RamseyN := Nat.find_spec (Ramsey₂Finite s t)
    apply Nat.find_le
...
theorem Ramsey₂StrictIneq : ∀ s t : ℕ, 0 < Ramsey₂ s t.succ + Ramsey₂ s.succ t → Even (Ramsey₂ s t.succ) → Even (Ramsey₂ s.succ t) → Ramsey₂ s.succ t.succ < Ramsey₂ s t.succ + Ramsey₂ s.succ t := by
  ...
  · rcases (Nat.eq_zero_or_eq_succ_pred (Ramsey₂ s.succ t)) with Rs1t | Rs1tsucc
    · have Ramsey0 := Nat.find_spec (Ramsey₂Finite s.succ t)
      unfold Ramsey₂ at Rs1t
      simp [Rs1t] at Ramsey0
      unfold Ramsey₂Prop at Ramsey0
      rcases (RamseyProp0 Ramsey0) with ⟨i, t0⟩
      fin_cases i <;> simp [List.Vector.get] at t0
      · cases s with
        | zero =>
        ...
        | succ s =>
          ...
          simp at evenM
    rw [Rst1succ, Nat.succ_add]
    apply Nat.lt_succ_of_le
    apply Nat.find_le
    simp
    rw [Rst1succ, Nat.succ_eq_add_one, Nat.even_add_one, Nat.not_even_iff_odd] at evenM
    rw [Rs1tsucc, Nat.succ_eq_add_one, Nat.even_add_one, Nat.not_even_iff_odd] at evenN
    have RamseyM : Ramsey₂Prop (Ramsey₂ s t.succ) s t.succ := by
      have RMem := Nat.find_spec (Ramsey₂Finite s t.succ)
      simp [Ramsey₂] at RMem ⊢
      exact RMem
    have RamseyN : Ramsey₂Prop (Ramsey₂ s.succ t) s.succ t := by
      have RMem := Nat.find_spec (Ramsey₂Finite s.succ t)
      simp [Ramsey₂] at RMem ⊢
      exact RMem
...
theorem Ramsey₂ByList : ∀ (N s t : ℕ), Ramsey₂Prop N s.succ t.succ ↔ (∀ (f : Sym2 (Fin N) → Fin 2), (∃ (l : List (Fin N)), l ∈ (List.finRange N).sublistsLen s.succ ∧ (List.Pairwise (λ u v ↦ (f s(u, v) )= 0) l)) ∨ (∃ (l : List (Fin N)), l ∈ (List.finRange N).sublistsLen t.succ ∧ (List.Pairwise (λ u v ↦ (f s(u, v)) = 1) l))) := by
  ...
  · intros h f
    ...
    rcases h f with ⟨S, SProp⟩
    let Slist := Finset.sort S instLEFin.le
    have hs: ∀ {i : Fin 2}, Symmetric fun u v => f (s(u, v)) = i := by
      unfold Symmetric
      simp [Sym2.eq_swap]
    rcases SProp with SProp | SProp
    left; swap; simp [List.Vector.get] at SProp; right;
    all_goals {
      use Slist
      apply And.intro
      · simp only [Slist, List.mem_sublistsLen, List.sublist_iff_exists_fin_orderEmbedding_get_eq, Finset.length_sort instLEFin.le]
        let f' : Fin Slist.length → Fin N := Slist.get
        ...
        have idxMapInj : Function.Injective idxMap := by
          ...
          intros a₁ a₂
          intro h
          simp [idxMap, f', Slist, Fin.cast, ← Fin.ext_iff] at h
          rw [← List.Nodup.get_inj_iff (Finset.sort_nodup S instLEFin.le)]
          exact h
        have idxOrdered : ∀ {a₁ a₂ : Fin Slist.length}, idxMap a₁ ≤ idxMap a₂ ↔ a₁ ≤ a₂ := by
          intros a₁ a₂
          simp [idxMap, Fin.cast]
          apply Iff.intro
          · intro valCond
            ...
            | isFalse p =>
              simp at p
              have a₂lea₁ := List.Pairwise.rel_get_of_lt (Finset.pairwise_sort S instLEFin.le) p
              have a₂eqa₁ := le_antisymm valCond a₂lea₁
              rw [List.Nodup.get_inj_iff (Finset.sort_nodup S instLEFin.le)] at a₂eqa₁
              simp [a₂eqa₁] at p
          · intro idCond
            exact List.Pairwise.rel_get_of_le (Finset.pairwise_sort S instLEFin.le) idCond
        simp [SProp.right]
        use { toFun := idxMap, inj' := idxMapInj, map_rel_iff' := idxOrdered }
        simp [Fin.cast, idxMap, f', Slist]
      · simp [Slist, ← List.pairwise_iff_coe_toFinset_pairwise (Finset.sort_nodup S instLEFin.le) hs, Set.Pairwise]
        -- TODO: At some point the simplification started introducing ¬x = y in the conclusion of SProp, find out why
        intros _ xinS _ yinS xneqy
        exact (SProp.left xinS yinS xneqy).right
    }
  · intros RamseyProp f
    ...
    have fSymm : Symmetric fun u v => f (Quot.mk (Sym2.Rel (Fin N)) (u, v)) = 1 := by
      ...
      simp[Sym2.eq_swap]
    all_goals{
      simp [SimpleGraph.isNClique_iff,SimpleGraph.isClique_iff, Set.Pairwise, graphAtColor, List.Vector.get]
      simp at lSubl
...
theorem friendship : Ramsey₂ 3 3 = 6 := by
  unfold Ramsey₂
  rw [Nat.find_upward_closed_eq_succ_iff]
  · simp
    apply And.intro
...
theorem Ramsey₂_binomial_coefficient_ineq : ∀ s t : ℕ, Ramsey₂ s.succ t.succ ≤ Nat.choose (s + t) s := by
  intros s t
  induction s generalizing t with
  | zero => simp [Ramsey₂1 t]
  | succ s' ihp₁ =>
    induction t with
    | zero =>
      rw [Ramsey₂Symm]
    ...
    | succ t' ihp₂ =>
      ...
      · apply Ramsey₂Ineq s'.succ t'.succ
        ...
        intro h
        simp at h
        have RamseyMem := Nat.find_spec (Ramsey₂Finite (Nat.succ s') (Nat.succ (Nat.succ t')))
        unfold Ramsey₂ at h
        rw [h.left] at RamseyMem
        simp [Ramsey₂Prop] at RamseyMem
        rcases RamseyProp0 RamseyMem with ⟨i, i0⟩
        fin_cases i <;> simp [List.Vector.get] at i0
      · have temp₁: Ramsey₂ s'.succ t'.succ.succ + Ramsey₂ s'.succ.succ t'.succ ≤ (s' + t'.succ).choose s' + (s'.succ + t').choose s'.succ := by
          apply add_le_add
          · exact ihp₁ t'.succ
          · exact ihp₂
        have temp₂ :(s'.succ + t'.succ).choose s'.succ = (s' + t'.succ).choose s' + (s'.succ + t').choose s'.succ := by simp [Nat.add_succ s' t', Nat.succ_add s' t', Nat.succ_add s' t'.succ, ← Nat.choose_succ_succ]
        rw [temp₂]
        exact temp₁
