...
--           apply And.intro
--           · -- prove < 325
--             fin_cases i <;> simp (config := { zetaDelta := true }) [ehyp]
--             · have w := block₁.2
--               simp [a₁eq]
--               linarith (transparency := .all)
--           · --prove color ≠ c
--             fin_cases i
--             · simp at ehyp
--               tauto
...
--                 simp_arith [a₁eq, a₃, Nat.add_assoc (5 * block₁.val) i₁]
...
--   apply And.intro
--   · simp (config := { zetaDelta := true, index := false }) [vdWProp, eq_iff_true_of_subsingleton]
--     use { start := 0, diff := 1}
--     simp
--     intros _ eins
